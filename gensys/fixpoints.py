...
# 2.1. Deterministic Product Game 
...
#   2.2.2. Antichain based (AC) Fixpoint Procedure
# 2.3. Bounded Fixpoint Procedures (Optimized)
#   2.3.1. On-The-Fly Determinization (OTFD) based Fixpoint Procedure
#   2.3.2. Antichain based (AC) Fixpoint Procedure
#-----------------------------------------------------------------------------------#
...
def buchi_fixedpoint_gensys(controller_moves, environment, guarantee, mode, game_type, init):
    ...
    #         # Add the last move condition, specific to Buchi: WP(G, W0) and C[1] and X
    #         W0__ = substitute(W0, *substList)
...
def cobuchi_fixedpoint_gensys(controller_moves, environment, guarantee, mode, game_type, init):
    ...
    while True:
        ...
        # Game Formulation for the safety game
        W1 = Or(getFormulation(s_, s__, controller, environment(*envtransitionVars), guarantee(*s_), W0_, "general"), W_Safety)
        g = Goal()
        g.add(W1)
        W1 = tactic_qe_fixpoint(g).as_expr()
        C.append(And(W1, Not(W0)))
...
def buchi_fixedpoint(controller_moves, environment, guarantee, mode, automaton, isFinal, nQ, game_type, init):
    ...
    odd_length_con_states = Exists(s_+[q_], And(controller, automaton(q,q_,*s), guarantee_automaton(q_)))
    g =Goal()
    g.add(odd_length_con_states)
    odd_length_con_states = tactic_qe_fixpoint(g).as_expr()

    weakened_guarantee = Or(odd_length_con_states, guarantee_automaton(q))
    ...
    while True:
        ...
        # WPW = And(getFormulation(s_, s__, controller, environment(*envtransitionVars), And(False), W0__, "general"), weakened_guarantee)
        WPW = And(Exists(s_+[q_], And(And(controller, automaton(q,q_,*s)), ForAll(s__+[q__], Implies(And(environment(*envtransitionVars), automaton(q_,q__,*s_)) , W0__)) )), weakened_guarantee)
        g =Goal()
        g.add(WPW)
        WPW = tactic_qe_fixpoint(g).as_expr()
        j = 1
        ...
        while True:
            ...
            H0__ = substitute(H0, *substList)
            # WPH = getFormulation(s_, s__, controller, environment(*envtransitionVars), And(False), H0__, "general")
            WPH = Exists(s_+[q_], And(And(controller, automaton(q,q_,*s)), ForAll(s__+[q__], Implies(And(environment(*envtransitionVars), automaton(q_,q__,*s_)) , H0__)) ))
            H1 = Not(And(Not(WPH), Not(WPW)))
            # H1 = Or(WPH, WPW)
            g =Goal()
            g.add(H1)
    ...
    # # print(W0)
    print("Game States equal?")
    assert (valid(WW == W0,1))
    print("KVR => Weakening?")
    assert (valid(Implies(WWpre, W0pre),1))
    print("KVR == Weakening?")
    assert (valid(WWpre == W0pre,1))
...
def cobuchi_fixedpoint(controller_moves, environment, guarantee, mode, automaton, isFinal, nQ, game_type, init):
    ...
    W0 = guarantee_automaton(q)
    W1 = guarantee_automaton(q)
    ...
    while True:
        ...
        # Game Formulation for the safety game
        W1 = And(Exists(s_+[q_], And(controller, automaton(q,q_,*s), guarantee_automaton(q_), ForAll(s__+[q__], Implies(And(environment(*envtransitionVars), automaton(q_,q__,*s_)), W0_)))), guarantee_automaton(q))
        g = Goal()
        g.add(W1)
        W1 = tactic_qe_fixpoint(g).as_expr()
    ...
    while True:
        ...
        # Game Formulation for the safety game
        W1 = Or(Exists(s_+[q_], And(controller, automaton(q,q_,*s), ForAll(s__+[q__], Implies(And(environment(*envtransitionVars), automaton(q_,q__,*s_)), W0_)))), W_Safety)
        g = Goal()
        g.add(W1)
        W1 = tactic_qe_fixpoint(g).as_expr()
        # C.append(And(W1, Not(W0)))
...
# -----------------------------------------------------------------------------------------
# -----------------------------------------------------------------------------------------
# 2.3. Bounded Fixpoint Procedures (Optimized) (Without sigma)
# -----------------------------------------------------------------------------------------
# -----------------------------------------------------------------------------------------
...
# -----------------------------------------------------------------------------------------
# 2.3.1. On-The-Fly Determinization (OTFD) based Fixpoint Procedure: Non-sigma version
# -----------------------------------------------------------------------------------------
...
# --------------------------------------------
# 2.3.2. Antichain based (AC) Fixpoint Procedure: Non-sigma version
# --------------------------------------------
