...
query_naming(s3('_b', '_b1', '_b2', '_b3', '_b4', '_b5',
		b1, b2, b3, b4, b5, 'b\'', 'b1\'', 'b2\'', 'b3\'', 'b4\'', 'b5\'')).

init(B, B1, B2, B3, B4, B5) := B>=0, B1=0, B2=0, B3=0, B4=0, B5=0, B=14.
overflow(B, B1, B2, B3, B4, B5) := B1>B; B2>B; B3>B; B4>B; B5>B.
stepmother(B, B1, B2, B3, B4, B5, B, B1p, B2p, B3p, B4p, B5p) := B1p>=B1, B2p>=B2, B3p>=B3, B4p>=B4, B5p>=B5, B1p+B2p+B3p+B4p+B5p-B1-B2-B3-B4-B5=1.
...
% fresh skolem relation s3(G,H,I,J,K,L,  M,N,O,P,Q,  A,B,C,D,E,F) 
% (exists [A,B,C,D,E,F]: inv(A,B,C,D,E,F),round(G,H,I,J,K,L,A,B,C,D,E,F)) :- inv(G,H,I,J,K,L),\+ (H>G;I>G;J>G;K>G;L>G),(M=0,N=0,O=J,P=K,Q=L;M=H,N=0,O=0,P=K,Q=L;M=H,N=I,O=0,P=0,Q=L;M=H,N=I,O=J,P=0,Q=0;M=0,N=I,O=J,P=K,Q=0).
skolem_template(s3, [_B, _B1, _B2, _B3, _B4, _B5,
		     B1, B2, B3, B4, B5,
