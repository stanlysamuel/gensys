...
for file in $FILES; do
  ...
  OUTPUT=`(time timeout $TIMELIMIT ./ctlstar $EXTRAOPTS $file > $logfile) 2>&1`
...
#  TODO: reenable when MCC is in place
  ...
  TIME=`echo "$OUTPUT" | grep "^real" | perl -pe 's/^real\s+(\d+)m(\d+)\.(\d+)s\$/(\$1*60+\$2)*1000+\$3/e'`
  echo " (in" `printf "%i" "$TIME"` "ms)"
done
