...
if spec == "product":
    # Spec: Liveness, G(1<=x<=4) and G(F(x==1) and F(x==2) and F(x==3) and F(x==4))
    ...
    if spec == "bounded":
        ...
        def automaton(q, q_, x):
            return Or(
                    And(q == 0, q_==0, And(x>=1, x<=4)),
                    And(q == 0, q_==1, Not(And(x>=1, x<=4))),
                    And(q == 1, q_==1),
                    And(q == 0, q_==2, And(x>=1, x<=4, x!=1)),
                    And(q == 2, q_==2, x!=1),
                    And(q == 2, q_==6, x==1),
                    And(q == 0, q_==3, And(x>=1, x<=4, x!=2)),
                    And(q == 3, q_==3, x!=2),
                    And(q == 3, q_==6, x==2),
                    And(q == 0, q_==4, And(x>=1, x<=4, x!=3)),
                    And(q == 4, q_==4, x!=3),
                    And(q == 4, q_==6, x==3),
                    And(q == 0, q_==5, And(x>=1, x<=4, x!=4)),
                    And(q == 5, q_==5, x!=4),
                    And(q == 5, q_==6, x==4),
                    And(q == 6, q_==6),
                    )
        ...
        # Call the fixpoint engine for omega regular specifications.
        # for i in range(0,7):
        otfd_fixedpoint(controller_moves, environment, guarantee, int(mode), automaton, isFinal, sigma, nQ, 6, game_type, init)
