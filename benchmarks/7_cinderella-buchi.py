...
# Cinderella-Stepmother game of 5 buckets with bucket size of C = 1.4. Here, the controller (protagonist) is Cinderella and it aims to win the game by satisfying a buchi specification. 
# Specification: Buchi, GF(And(b1 <= C , b2 <=C , b3 <=C , b4 <=C , b5 <=C , b1 >= 0.0 , b2 >= 0.0 , b3 >= 0.0 , b4 >= 0.0 , b5 >= 0.0))
...
if spec_type == "simple":
    ...
    buchi_fixedpoint_gensys(cinderella_moves, stepmother, guarantee, 0, game_type, init)
    ...
    if spec_type == "product-buchi":
        ...
        if spec_type == "product-co-buchi":
            # Not applicable for Buchi specifications as co-buchi automatons for buchi specifications are non-deterministic.
            ...
            if spec_type == "otf":
                ...
                # Call the fixpoint engine for omega regular specifications (k = 1).
                for i in range(0, 2):
                    otfd_fixedpoint(cinderella_moves, stepmother, guarantee, 0, automaton, isFinal, sigma, nQ, i, game_type, init)
                    # otfd_fixedpoint_nonsigma(cinderella_moves, stepmother, guarantee, 0, automaton, isFinal, sigma, nQ, i, game_type, init)
