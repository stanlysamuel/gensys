...
# EAA algorithm for infinite games with LTL properties - Elevator example
# With counters
...
def automaton(q, q_, x):
    return Or(
            ...
            And(q == 3, q_==4, x == 2),
            And(q == 4, q_==4, x == 2),
            #For completeness, dead state q5
            And(q == 1, q_==5, x!=1),
            And(q == 2, q_==5, x!=2),
            And(q == 3, q_==5, x!=2),
            And(q == 4, q_==5, x!=2),
...
def controller(x, x_):
    return Or(x_== x+1, x_ == x)

# 3. Define Guarantee
def guarantee(q, x, c):
    return And(c>=0, c<=1, q>=1, q<=5, x>=1, x<=2)

# 4. Define initial conditions of the automata
def init(q, x, c):
    return And(q==1, c==0)

# 5. Define counter constraints

def counter_constraints(q_, c, c_):
    return  And(
            Implies(Or(q_==2, q_==5), c_ == c + 1),
            Implies(Not(Or(q_==2, q_==5)), c_ == c)
    )
...
wpAssertion = Exists([x_], ForAll([s_, c_], And(controller(x,x_), Implies(And(automaton(s,s_,x), counter_constraints(s_, c, c_)), And(guarantee(s_, x_, c_), ForAll([x__, s__, c__], Implies(And(environment(x_, x__), automaton(s_, s__, x_), counter_constraints(s__, c_, c__)), guarantee(s__,x__, c__))))))))
...
while(not valid(Implies(F, W))):
    ...
    wpAssertion = Exists([x_], ForAll([s_, c_], And(controller(x,x_), Implies(And(automaton(s,s_,x), counter_constraints(s_, c, c_)), And(guarantee(s_, x_, c_), ForAll([x__, s__, c__], Implies(And(environment(x_, x__), automaton(s_, s__, x_), counter_constraints(s__, c_, c__)), W)))))))
...
#Conjunct winning region with initial conditions
if not satisfiable(And(F, init(s,x,c))):
    ...
    if check_conflict(F, controller, automaton) == unsat:
        ...
        condition_move3 = Exists([x_], ForAll([s_, c_], And(x_ == x, Implies(And(automaton(s,s_,x), counter_constraints(s_, c, c_)), And(guarantee(s_, x_, c_), ForAll([x__, s__, c__], Implies(And(environment(x_, x__), automaton(s_, s__, x_), counter_constraints(s__, c_, c__)), F)))))))
        ...
        #Eliminate qe and conjunct with guarantee
        condition_move3 = And(tactic_qe_controller(g).as_expr(), guarantee(s, x,c))
        g = Goal()
        g.add(condition_move3)
        #Simplify to get final condition
        condition_move3 = tactic_simplification(g).as_expr()
        condition_move3 = tactic_qe_fixpoint(g).as_expr()
        ...
        g = Goal()
        g.add(Exists([s,c], condition_move3))
        condition_move3_ = tactic_qe_fixpoint(g).as_expr()
        print("Projected move 3 is: ")
        print(condition_move3_)
        print("")
        ...
        formula = Or(condition_move1, condition_move2, condition_move3) == Invariant
        valid(formula)
