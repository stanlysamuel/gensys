...
def succ(c, sigma_x, c_):
    ...
    reach = And([Implies(c_[q] != -1,  Or([Exists([x], And(automaton(p,q,x), sigma_x, c[p] != -1)) for p in range(0, len(c))])) for q in range(0, len(c_))])

    # 3. Determinization constraint

    # This constraint gives the count on valid target states that has >1 input states.
    det1 = And([And([And([ForAll([x], Implies(And(automaton(p,q,x), sigma_x, c[p] != -1, automaton(p_,q,x), sigma_x, c[p_] <= c[p], c[p_] != -1, p!=p_), c_[q] == min(c[p] + isFinal(q) , k+1))) for p_ in range(0, len(c))]) for p in range(0, len(c))]) for q in range(0, len(c_))])

    # This constraint gives the count on valid target states that has 1 input state.
    det2 = And([And([ForAll([x], Implies(And(automaton(p,q,x), sigma_x, c[p] != -1, Not(Or([ And(automaton(p_,q,x), sigma_x, c[p_] != -1, p!=p_) for p_ in range(0, len(c))]) )), c_[q] == min(c[p] + isFinal(q) , k+1))) for p in range(0, len(c))]) for q in range(0, len(c_))])
...
def Succ(c, x_subst, c_):
    return Or([And( substitute(sigma[i], (x, x_subst)), succ(c,sigma[i],c_) ) for i in range(len(sigma))])
...
while(not valid(Implies(F, W))):
    ...
    W = substitute(W, (x, x__), *[(c[j], c__[j]) for j in range(nQ)])
